...
export default Form.create<FormProps>()(
  class extends React.Component<FormProps, { option; current }> {
    constructor(props) {
      super(props);
      ...
      form.validateFields(async (err, values) => {
        ...
        switch (this.state.current) {
          case "wealth":
            ...
            response = await Utility.awardWealth(
              reason,
              value,
              this.props.item.id
            );
          ...
          case "prestige":
            ...
            reason = values.reason.toString();
            response = await Utility.addPrestige(
              this.props.item.id,
              value,
              reason
            );
          ...
          case "punishwealth":
            value = parseInt(values.wealth);
            reason = values.reason.toString();
            response = await Utility.deductWealth(
              reason,
              value,
              this.props.item.id
            );
          ...
          case "punishprestige":
            value = parseInt(values.prestige);
            reason = values.reason.toString();
            response = await Utility.deductPrestige(
              this.props.item.id,
              value,
              reason
            );
          ...
          case "delete":
            reason = values.reason.toString();
            response = await Utility.deletePost(
              this.props.item.topicId,
              this.props.item.id,
              reason
            );
            break;
          case "tp":
            ...
            response = await Utility.stopBoardPost(
              this.props.item.id,
              reason,
              value
            );
            break;
          case "canceltp":
            response = await Utility.cancelStopBoardPost(
    ...
    render() {
        ...
        case "wealth":
          rcForm = (
            <Form layout="vertical">
              <Tag color="blue">
                您今天在{this.props.boardName}已经发了{this.props.d_wealth}
                财富值，最多可发{m_wealth} （单次最多1000）
              </Tag>
              <FormItem label="财富值">
      ...
      return (
        ...
        >
          ...
          >
            <Menu.Item key="wealth">奖励财富值</Menu.Item>
            <Menu.Item key="prestige">奖励威望</Menu.Item>
            <Menu.Item key="punishwealth">扣除财富值</Menu.Item>
            <Menu.Item key="punishprestige">扣除威望</Menu.Item>
            <Menu.Item key="delete">删除</Menu.Item>
            <Menu.Item key="tp">TP</Menu.Item>
            <Menu.Item key="canceltp">解除TP</Menu.Item>
          </Menu>
          {rcForm}
        </Modal>
      );
