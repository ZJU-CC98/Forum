...
export class BoardList extends React.Component<{}, { data: RootBoardState[] }> {
  ...
  generateRootBoard(data: RootBoardState) {
    //返回一条父版信息
    return <RootBoard key={data.id} data={data} />;
  }
  generateBoardGuide(item: RootBoardState) {
    const name = `${item.name}`;
    return (
      <div
        key={item.id}
        onClick={() => this.scroll(name)}
        className="row boardOption"
      >
        {item.name}
      </div>
    );
...
export class RootBoard extends React.Component<
  { data: RootBoardState },
  { isExpanded: boolean }
> {
  constructor(props) {
  ...
  render() {
    ...
    return (
      <div className="anArea" id={data.name}>
          ...
          <div className="row boardListHead" onClick={this.toggleIsExpanded}>
            ...
            >
              ...
              <div className="areaName">
                {mastersText}
                {data.masters.map(this.convertMasters)}
              </div>
            </div>
...
export class ChildBoard extends React.Component<
  { id: number; boards: ChildBoardState[] },
  {}
> {
  constructor(props) {
    super(props);
  ...
  convertChildBoard(item: ChildBoardState) {
    ...
    return (
      <div key={item.id} className="boardContent">
        <Link to={`/board/${item.id}`}>
          <div className="greenBackdrop">
            <img
  ...
  convertNoImgChildBoard(item: ChildBoardState) {
    return (
      <div key={item.id} className="noImgBoardContent">
        <Link to={`/board/${item.id}`}>
          <div className="boardName2">{item.name}</div>
        </Link>
      </div>
    );
  }
