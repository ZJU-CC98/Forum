 ...
 * 表示我关注的版面的主题列表
 */
export class FocusBoardTopicArea extends React.Component<FocusBoardTopicProps, FocusTopicAreaState> {
    ...
    constructor(props) {
        super(props);
        //先看一下有没有缓存的帖子数据
        var data = Utility.getStorage(`focusBoard_${this.props.boardId}`);
        if (!data) {
            data = [];
        }
        this.state = {
    ...
    async handleScroll() {
        if (Utility.isBottom() && this.state.loading) {
            ...
            */
            ...
            try {
                var newData = await Utility.getFocusBoardTopic(this.props.boardId, this.props.boardNme, this.state.from, this.context.router);
            } catch (err) {
                /**
...
export class FocusBoardTopicProps {
    boardId: number;
    boardNme: string;
}