...
> {
  ...
  async getPhotoFrame(displayTitleId: number) {
    ...
    if (this.props.topicInfo.isAnonymous == true) {
      ...
      let shadow = {}; //头像框底部的阴影
      if (displayTitleId === 82) shadow = { boxShadow: '0 0 0' }; //吉祥物无阴影
  ...
  render() {
    ...
    if (!this.props.topicInfo.isAnonymous) {
      ...
      urlHtml = (
        <a href={realUrl} style={{ display: 'block', maxHeight: '7.5rem' }}>
          <img className="userPortrait" src={this.props.userInfo.portraitUrl} />
        </a>
      );
    }
    ...
    const lastLogOn = dayjs(this.props.userInfo.lastLogOnTime).fromNow();
    ...
    let gender = (
      <div className="userGender">
        {this.props.userInfo.gender === 0 ? (
          <i className="fa fa-venus" style={{ color: '#fff' }} />
        ) : (
          <i className="fa fa-mars" style={{ color: '#fff' }} />
        )}
      </div>
    );
    if (this.props.topicInfo.isAnonymous == true) {
            ...
            <div className="row" style={{ width: "100%", display: "flex", marginBottom: "0.625rem" }}>
                ...
                <div className="column" id="rpymes" >
                    ...
                    <div className="row" style={{ alignItems: "center", display: "flex", flexWrap: "nowrap" }}>
                        ...
                        <div><span className="timeProp">{dayjs(this.props.replyTime).format('YYYY-MM-DD HH:mm:ss')}</span></div>
