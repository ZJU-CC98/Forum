...
export default class extends React.Component<
  { id: number },
  UserCenterExactActivitiesPostsState
> {
  constructor(props) {
    super(props);
    this.state = {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          this.setState((prevState) => ({
            userRecentPosts: prevState.userRecentPosts.concat(
              posts.slice(0, i)
            ),
            isLoading: false,
          }));
  ...
  render() {
    ...
    //state转换为JSX
    const userRecentPosts = this.state.userRecentPosts.map((item) => (
      <Topic userRecentTopic={item} />
    ));
    //添加分隔线
    ...
    return (
      <div className="user-activities">
        <p>发表的主题</p>
        <div className="user-posts">{userRecentPosts}</div>
      </div>
    );
  }
}
