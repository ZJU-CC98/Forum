 ...
 * @author AsukaSong
 */
export const getRecentTopics: ActionCreator<ThunkAction<Promise<Action>, RootState, void, RootAction>> = (page: number) => async (dispatch, getState) => {
    try {
 ...
 * @author AsukaSong
 */
export const getRecentPosts: ActionCreator<ThunkAction<Promise<Action>, RootState, void, RootAction>> = (page: number, ishot: number) => async (dispatch, getState) => {
    try {
        dispatch(Actions.userCenterLoading());
        ...
        // 请求11条信息
        const url =`/me/${ishot === 0 ? 'recent' : 'hot'}-post?from=${(page - 1) * 10}&size=11`;
        const headers = await Utility.formAuthorizeHeader();
        const res = await Utility.cc98Fetch(url, { headers });
        if (res.status !== 200) { throw new Error(res.statusText); }
        ...
        dispatch(Actions.changeUserRecentPosts({posts: recentPosts, total} as PostsAndTotal));
        return dispatch(Actions.userCenterLoaded());
    } catch (e) {
        return dispatch(Actions.userCenterError(e.message));
    }
 ...
 * @author AsukaSong
 */
export const getHotPosts: ActionCreator<ThunkAction<Promise<Action>, RootState, void, RootAction>> = (page: number) => async (dispatch, getState) => {
    try {
        dispatch(Actions.userCenterLoading());
        const recentPosts = getState().userInfo.recentPosts;
        const hasTotal = getState().userInfo.hasTotal.myposts;
        ...
        dispatch(Actions.changeUserHotPosts({posts: recentPosts, total} as PostsAndTotal));
        return dispatch(Actions.userCenterLoaded());
    } catch (e) {
        return dispatch(Actions.userCenterError(e.message));
    }
