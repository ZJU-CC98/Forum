    ...
    do {
      ...
      for (const segment of subSegments) {
        if (isQuoteTag(segment)) {
          // 把下一层的 [quote] 标签移除
          const [quoteSegment] = subSegments.splice(
            subSegments.indexOf(segment),
            1
          );
    ...
    if (queue.length > 1) {
      ...
      for (let i = 1; i < queue.length; i++) {
        if (isPureQuote(queue[i].subSegments)) {
          let info = (queue[i].subSegments.shift() as Ubb.UbbTagSegment).clone(
            queue[i - 1]
          );
          queue[i - 1].subSegments.unshift(info);
          queue[i - 1].subSegments.splice(
            1,
            0,
            new Ubb.UbbTextSegment("\n", queue[i - 1])
          );
          queue.splice(i, 1);
          i--;
        }
...
class Quote extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    const showQuotes = isShowed
      ? quoteItems
      : quoteItems.slice(quoteItems.length - 2);
