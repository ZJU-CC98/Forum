...
export class Search extends React.Component<{}, SearchState> {
  ...
  async getData(searchInfo: any, from: number) {
    let newTopic = await Utility.getSearchTopic(
      searchInfo.boardId,
      searchInfo.words,
      from,
      this.context.router
    ...
    if (!newTopic || newTopic.length === 0) {
      ...
      //搜索结果小于20条，无法再获取新的了,添加新数据，this.isLoadable设置为false，后续不可以再次发送fetch请求
      ...
      //搜索结果多于20条，还可以通过滚动条继续获取,this.isLoadable设置为true，后续可以再次发送fetch请求
      else {
        let data = this.state.data.concat(newTopic);
        this.setState(
          {
      ...
      <div className="focus-root">
        ...
        <div className="focus">
          ...
          <div className="focus-topic-area" id="focus-topic-area">
            ...
            <div
              className="focus-topic-getMore"
              onClick={this.getMore}
              id="focus-topic-getMore"
            >
              <div>点击获取更多搜索结果~</div>
              <div>······</div>
            </div>
            <div
              className="focus-topic-loading displaynone"
              id="focus-topic-loading"
            >
              <img src="http://file.cc98.org/uploadfile/2017/12/20/6514723843.gif"></img>
            </div>
            <div
          ...
          <div id="noResult" className="noResult displaynone">
            <img
              src="/static/images/searchNone.png"
              className="noResultPic"
            ></img>
            <div className="noResultText">
              -----------------------抱歉呢前辈，没有找到你想要的帖子哦~----------------------
            </div>
          </div>
          <div id="showError" className="resultErr displaynone">
            查询出错了，请刷新重试
          </div>
        </div>
      </div>
