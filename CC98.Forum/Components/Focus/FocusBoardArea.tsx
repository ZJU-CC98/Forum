 ...
 * 表示我关注的版面列表区域
 */
export class FocusBoardArea extends React.Component<
  { history },
  FocusBoardAreaState
> {
  /**
  ...
  convertFocusBoard = (item: FocusBoard) => {
    //点击版面名称会显示相应版面的帖子
    return (
      <div key={item.id} className="focus-board" id={`focusBoard_${item.id}`}>
        {item.name}
      </div>
    );
  };
    ...
    return (
      <div>
        <div className="focus-board-area">
          <div className="focus-board" id="focusBoard_0">
            全部帖子
          </div>
          {this.state.data.map(this.convertFocusBoard)}
          <div className="focus-boardTip">
            提示：单击版面标签可切换下方展示内容，双击可直接进入版面
          </div>
        </div>
        <FocusTopicArea
          id={this.state.currentBoardId}
          name={this.state.currentBoardName}
        />
      </div>
    );
  }
