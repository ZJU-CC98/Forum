...
export class TopicInfo extends React.Component<
  Props,
  { tag1Name; tag2Name; isFollow; showImageState: boolean; followCount: number }
> {
  constructor(props) {
    super(props);
    ...
    this.state = {
      tag1Name: "",
      tag2Name: "",
      isFollow: this.props.isFav,
      showImageState: true,
      followCount: this.props.topicInfo.favoriteCount,
    };
  }
  ...
  async follow() {
    ...
    confirm({
      ...
      onOk: async (e) => {
        console.log(select);
        try {
          await Utility.setFavoriteTopic(this.props.topicInfo.id,select);
  ...
  async componentDidMount() {
    let t1 = "",
      t2 = "";
    if (this.props.tag1) t1 = await Utility.getTagNamebyId(this.props.tag1);
    if (this.props.tag2) t2 = await Utility.getTagNamebyId(this.props.tag2);
  ...
  async componentWillReceiveProps(newProps) {
    let t1 = "",
      t2 = "";
    if (newProps.tag1) t1 = await Utility.getTagNamebyId(newProps.tag1);
    if (newProps.tag2) t2 = await Utility.getTagNamebyId(newProps.tag2);
    this.setState({ tag1Name: t1, tag2Name: t2 });
  }
  onError(e) {
    e.preventDefault();
  ...
  render() {
    ...
    if (this.props.tag1 || this.props.tag2) {
      tags = (
        <div id="tags">
          <div className="tagProp tagSize">
            标签： {this.state.tag1Name} {this.state.tag2Name}
          </div>
          <div className="tagProp"></div>
        </div>
      );
    }
    // 显示/收起所有图片按钮
    ...
    return (
      <div className="topicInfo-info">
        <div className="topicInfo-title">
          <div className="column" id="topicTitleProp">
            <div
              id="essay1"
              className="row"
              title={title.length > overflowLen ? title : undefined}
            >
              {title.length <= overflowLen
                ? title
                : title.slice(0, overflowLen) + " ..."}
            </div>
            <div className="row" id="essayProp">
              {tags}
              <div id="time">
                <div className="viewProp">
                  <i className="fa fa-clock-o fa-lg fa-fw"></i>
                </div>{" "}
                <div className="timeProp tagSize">
                  {moment(this.props.topicInfo.time).format(
                    "YYYY-MM-DD HH:mm:ss"
                  )}
                </div>
              </div>
              <div id="viewtimes">
                <div className="viewProp">
                  <i className="fa fa-eye fa-lg fa-fw"></i>{" "}
                </div>{" "}
                <div className="timeProp tagSize">
                  {this.props.topicInfo.hitCount}
                </div>
              </div>
              <div id="followCount">
                <div className="viewProp">
                  {this.state.isFollow ? (
                    <i className="fa fa-star fa-lg fa-fw"></i>
                  ) : (
                    <i className="fa fa-star-o fa-lg fa-fw"></i>
                  )}{" "}
                </div>{" "}
                <div className="timeProp tagSize">{this.state.followCount}</div>
              </div>
              <div
                className="followTopic"
